...
class RobustRegression(Regression):
    ...
    method='huber':
        ...
        IRLS weight: w_n = 1 if |r_n| <= delta, else delta / |r_n|.
        ``epsilon`` controls the Huber threshold multiplier (default 1.35).
    method='student_t':
        IRLS with Student-t loss. Residuals are scaled by MAD before weighting.
        IRLS weight: w_n = (nu + 1) / (nu + r_scaled_n^2).
        ``nu`` controls the degrees of freedom (default 4.0); smaller nu gives
        heavier tails and more aggressive downweighting of outliers.
    method='ransac':
    ...
    def _fit_student_t(self, X: np.ndarray, y: np.ndarray) -> None:
        """IRLS with Student-t loss (heavy-tailed, more robust than Huber to extreme outliers).

        IRLS weight: w_n = (nu + 1) / (nu + r_scaled_n^2)
        where r_scaled_n = r_n / sigma_MAD.

        The negative log-likelihood contribution tracked in loss_history_ is:
            ((nu+1)/2) * log(1 + r_scaled^2 / nu)
        """
        X = np.asarray(X, dtype=float)
        y = np.asarray(y, dtype=float).reshape(-1)
        X_b = self._augment(X)
        ...
        for step in range(1, self.max_iter + 1):
            ...
            # IRLS weight for Student-t: w_n = (nu+1) / (nu + r_scaled^2)
            weights = (nu + 1.0) / (nu + r_scaled ** 2)
            weights /= np.mean(weights)

            theta_new = self._solve_weighted(X_b, y, weights, self.alpha)
            update_norm = float(np.linalg.norm(theta_new - theta))

            # Negative log-likelihood (up to constants): ((nu+1)/2) * log(1 + r^2/nu)
            loss = float(np.mean(((nu + 1.0) / 2.0) * np.log1p(r_scaled ** 2 / nu)))
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs) -> None:  # type: ignore[override]
        if self.method == "huber":
            ...
            self._fit_student_t(X, y)
        elif self.method == "ransac":
            self._fit_ransac(X, y)
        else:
            raise ValueError(
                f"Unknown method {self.method!r}. Choose 'huber', 'student_t', or 'ransac'."
            )
