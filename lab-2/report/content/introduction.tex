...
\begin{notation}
\begin{itemize}
  \item \textbf{Tập hợp và không gian:} $\mathcal{X}$ là không gian mẫu, $\mathcal{H}$ là không gian Hilbert, $P_k$ là tập ma trận chiếu hạng $k$, $\mathcal{N}_i$ là tập láng giềng của điểm $i$.
  \item \textbf{Mẫu và chiều:} $S=(x_1,\ldots,x_m)$ gồm $m$ mẫu; $N$ là số chiều gốc, $k$ là số chiều sau giảm chiều, $d$ là số chiều trực quan hóa khi cần.
  \item \textbf{Vector:} chữ thường in đậm, ví dụ $\mathbf{x}_i=\Phi(x_i)\in\R^N$, $\mathbf{y}_i\in\R^k$, $\mathbf{1}$ là vector toàn $1$.
  \item \textbf{Ma trận:} chữ hoa in đậm, ví dụ $\mathbf{X}\in\R^{N\times m}$ là ma trận dữ liệu, $\mathbf{Y}\in\R^{k\times m}$ là embedding, $\mathbf{I}_m$ là ma trận đơn vị.
  \item \textbf{Ma trận cốt lõi:} $\mathbf{C}$ là hiệp phương sai, $\mathbf{K}$ là Gram/kernel, $\widetilde{\mathbf{K}}$ là kernel đã căn giữa, $\mathbf{H}$ là ma trận centering, $\boldsymbol{\Delta}$ là ma trận khoảng cách bình phương.
  \item \textbf{Đồ thị và manifold:} $\mathbf{W}$ là ma trận trọng số, $\mathbf{D}$ là ma trận bậc, $\mathbf{L}$ là Laplacian, $\mathbf{M}$ là ma trận LLE, $\mathbf{K}_{\mathrm{Iso}}$ là Gram matrix của Isomap.
  \item \textbf{Phổ và phân rã:} $\lambda_i$ là trị riêng, $\sigma_i$ là giá trị kỳ dị, $\mathbf{u}_i,\mathbf{v}_i$ là vector riêng hoặc vector kỳ dị tùy ngữ cảnh; $\boldsymbol{\Lambda}$ và $\boldsymbol{\Sigma}$ là các ma trận đường chéo tương ứng.
  \item \textbf{Phân phối và hàm mục tiêu:} $p_{ij},q_{ij}$ là xác suất cặp điểm trong t-SNE; $B_{ij},w_{ij}$ là trọng số mờ của UMAP; $\mathcal{L}$ là hàm loss, $Z$ là hằng số chuẩn hóa hoặc hàm phân hoạch.
  \item \textbf{Toán tử:} $\|\cdot\|_F$ là chuẩn Frobenius, $\operatorname{Tr}(\cdot)$ là trace, $\Phi$ là ánh xạ đặc trưng. Các ký hiệu đặc thù hơn được định nghĩa lại tại nơi xuất hiện.
\end{itemize}
\end{notation}
...
\begin{enumerate}
  ...
  \item Chương~5: thực nghiệm PCA, KPCA, Isomap, LLE, Laplacian Eigenmaps, t-SNE và UMAP trên bốn bộ dữ liệu; so sánh trực quan, chỉ số định lượng và thời gian chạy.
