...
\paragraph{Các khái niệm nền tảng.}
...
\emph{Ví dụ:} Mặt quả cầu là đa tạp 2 chiều nhúng trong $\R^3$---mỗi điểm cần 3 tọa độ $(x,y,z)$, nhưng chỉ cần 2 tham số (kinh độ, vĩ độ) là xác định được vị trí.
...
\emph{Tô-pô} của một tập hợp mô tả ``hình dạng'' bản chất của nó---những tính chất không thay đổi khi ta bẻ cong, kéo giãn (nhưng không cắt, dán). Trong ngữ cảnh UMAP, ``giữ cấu trúc tô-pô'' nghĩa là giữ \emph{quan hệ kết nối}: ai là láng giềng của ai, dữ liệu chia thành mấy cụm, cụm nào nối với cụm nào.
\end{definition}
...
\textbf{Tính gradient.} Đặt $\mathbf{r}_{ij} = \mathbf{y}_i - \mathbf{y}_j$. Trước tiên tính đạo hàm của $w_{ij}$ theo $\mathbf{y}_i$:
\[
...
\end{enumerate}
Learning rate $\eta$ giảm tuyến tính từ $1$ về $0$ qua các epoch (tương tự simulated annealing: bước lớn lúc đầu để thoát cực tiểu cục bộ, bước nhỏ lúc sau để tinh chỉnh).
