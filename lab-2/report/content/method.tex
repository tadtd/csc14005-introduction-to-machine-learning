...
Hiện thực của nhóm dùng thin SVD, do đó nhận
$k\leq\min(m,N)$; giới hạn này phù hợp với bài toán giảm chiều đang xét.
...
\begin{enumerate}
  ...
  \item Trong hiện thực, nếu đồ thị bị rời thành nhiều thành phần, nối dần
  hai thành phần gần nhất bằng cạnh Euclidean ngắn nhất để có thể tính
  khoảng cách hữu hạn giữa mọi cặp điểm.
...
đứt rời, buộc bước nối thành phần trong hiện thực thêm cạnh ngoài đồ thị
láng giềng ban đầu; $t$ quá lớn lại tạo các cạnh tắt xuyên qua manifold.
...
\[
\Pr\!\left(\left|\frac{S}{k} - 1\right| > \varepsilon\right) \leq 2 e^{-\varepsilon^2 k/8}.
\]
\end{lemma}
...
\]
Chọn $t = \varepsilon/(2(1+\varepsilon))$ và dùng
$\ln(1+\varepsilon) \leq \varepsilon-\varepsilon^2/4$ (với
$\varepsilon \in (0,1)$) suy ra
$\Pr(S > (1+\varepsilon)k) \leq e^{-\varepsilon^2 k/8}$.
...
Vì $e^{-\varepsilon^2 k/4}\leq e^{-\varepsilon^2 k/8}$, union bound trên
hai đuôi cho
$\Pr(|\frac{S}{k}-1|>\varepsilon)\leq 2e^{-\varepsilon^2 k/8}$.
\end{proof}

\begin{theorem}[Johnson--Lindenstrauss]
\label{lem:jl}
Cho $0 < \varepsilon < 1$ và tập hữu hạn $\mathcal{V} = \{\mathbf{v}_1, \ldots, \mathbf{v}_m\} \subset \R^N$. Nếu $k > \frac{16 \log m}{\varepsilon^2}$, thì tồn tại ánh xạ $f : \R^N \to \R^k$ sao cho mọi $\mathbf{u}, \mathbf{v} \in \mathcal{V}$,
\[
...
\leq \|f(\mathbf{u}) - f(\mathbf{v})\|^2
\leq (1+\varepsilon)\|\mathbf{u}-\mathbf{v}\|^2
\right) \geq 1 - 2 e^{-\varepsilon^2 k/8}.
\]
...
\leq \binom{m}{2} \cdot 2 e^{-\varepsilon^2 k/8}
< m^2 e^{-\varepsilon^2 k/8}.
\]
Để $\Pr(A) > 0$ (tức tồn tại $\mathbf{R}$ thỏa mọi cặp), cần
$m^2 e^{-\varepsilon^2 k/8} < 1$, tương đương
\[
k > \frac{8 \cdot 2 \log m}{\varepsilon^2}
= \frac{16 \log m}{\varepsilon^2}.
\]
Vậy với $k$ như giả thiết định lý, xác suất chọn được $\mathbf{R}$ phù hợp là dương, nên ánh xạ $f$ như vậy tồn tại.
\end{proof}

\paragraph{Ý nghĩa.}
JL cho phép giảm $N$ xuống $k \approx \log m$ (bỏ qua hệ số
$1/\varepsilon^2$) mà vẫn giữ hình học cặp điểm---cơ sở cho
\emph{random projection} làm tiền xử lý nhanh, không cần SVD. Trong sách
\cite{mohri2018foundations} dùng hằng số $20$; chứng minh Chernoff và union
bound ở trên cho hằng số $16$, cùng bậc tiệm cận.

\begin{example}
Với $m = 1000$, $\varepsilon = 0{,}1$, ngưỡng
$k > 16\ln(1000)/0{,}01 \approx 1{,}1 \times 10^4$. Trên thực nghiệm
thường chọn $k$ nhỏ hơn nhiều vẫn chấp nhận được; bảo đảm JL là chặn đủ
cho trường hợp xấu nhất chứ không phải lựa chọn tối thiểu trong mọi dữ liệu.
\end{example}
