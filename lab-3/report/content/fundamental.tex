...
\begin{tikzpicture}[every node/.style={font=\small}]
  % --- Tensor cube 3 x 4 x 2 ---
  \begin{scope}[xshift=0cm, yshift=0cm]
    \drawtfgtensor{3}{4}{2}{0}{0}{pink!22}{magenta!70!black}
    \node[above=8pt, font=\footnotesize\bfseries] at (1.05,0.18)
      {$\mathcal{X}\in\mathbb{R}^{3\times4\times2}$};

    \draw[teal, line width=0.85pt, <->, >=stealth]
      (-0.55,0.02) -- (-0.55,-3*\tfgCellH-0.02);
    \node[text=teal, font=\footnotesize, anchor=east] at (-0.68,-0.75) {$I_1=3$};

    \draw[orange!85!black, line width=0.85pt, <->, >=stealth]
      (0.02,-3*\tfgCellH-0.35) -- (4*\tfgCellW-0.02,-3*\tfgCellH-0.35);
    \node[text=orange!85!black, font=\footnotesize, anchor=north] at (1.0,-3*\tfgCellH-0.43) {$I_2=4$};

    \draw[purple!75!black, line width=0.85pt, ->, >=stealth]
      (2.30,0.22) -- (1.85,0.42);
    \node[text=purple!75!black, font=\footnotesize, anchor=west] at (2.40,0.32) {$I_3=2$};
  \end{scope}

  % --- unfold arrow ---
  \draw[->, line width=1.1pt, gray!60, >=stealth] (3.05,-0.75) -- (4.35,-0.75);
  \node[font=\footnotesize, text=gray!65, fill=white, inner sep=1pt] at (3.70,-0.42) {unfold};

  % --- Matrix 3 x 8 ---
  \begin{scope}[xshift=4.80cm, yshift=0cm]
    \drawtfgface{3}{8}{0}{0}{teal!18}{teal!65!black}{0}
    \node[above=8pt, font=\footnotesize\bfseries, text=teal] at (2.0,0.18)
      {$\mathbf{X}_{(1)}\in\mathbb{R}^{3\times8}$};

    \draw[teal, line width=0.85pt, <->, >=stealth]
      (8*\tfgCellW+0.55,0.02) -- (8*\tfgCellW+0.55,-3*\tfgCellH-0.02);
    \node[text=teal, font=\footnotesize, anchor=west] at (8*\tfgCellW+0.68,-0.75) {$I_1=3$};

    \draw[teal, line width=0.85pt, <->, >=stealth]
      (0.02,-3*\tfgCellH-0.35) -- (8*\tfgCellW-0.02,-3*\tfgCellH-0.35);
    \node[text=teal, font=\footnotesize, anchor=north] at (2.0,-3*\tfgCellH-0.43) {$I_2I_3=8$};
  \end{scope}
\end{tikzpicture}
}
...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \pgfmathsetmacro{\labelY}{0.72}

  % ===== Hàng nhãn (căn theo tâm đồ họa bên dưới) =====
  \node[text=teal, font=\footnotesize\bfseries] at (0.25, \labelY)
    {$\mathbf{a}\in\mathbb{R}^I$};
  \node[text=orange!85!black, font=\footnotesize\bfseries] at (1.95, \labelY)
    {$\mathbf{b}\in\mathbb{R}^J$};
  \node[text=magenta!70!black, font=\footnotesize\bfseries] at (3.45, \labelY)
    {$\mathbf{c}\in\mathbb{R}^K$};
  \node[text=violet!80!black, font=\footnotesize\bfseries, align=center] at (6.35, \labelY)
  ...
  \node[font=\Large] at (1.05, \opCy) {$\circ$};

  \drawtfgveccy{3}{1.70}{\opCy}{orange!28}{orange!70!black}
  \node[font=\Large] at (2.75, \opCy) {$\circ$};

  \drawtfgveccy{2}{3.20}{\opCy}{pink!30}{magenta!55!black}
  \node[font=\Large] at (4.25, \opCy) {$=$};

  % Tensor 4 x 3 x 2
  \begin{scope}[xshift=5.35cm]
    \pgfmathsetmacro{\tTop}{\opCy+2*\tfgCellH}
    \drawtfgtensor{4}{3}{2}{0}{\tTop}{violet!18}{violet!65!black}
    \node[font=\scriptsize, text=violet!75!black, align=center] at (2.85, \opCy)
      {$\mathcal{X}_{i,j,k}=a_i b_j c_k$};
  \end{scope}
\end{tikzpicture}
}
...
\subsubsection{\texorpdfstring{Tích mode-$n$}{Tích mode-n}}
...
\end{equation}
Ta chứng minh $\mathbf{X}_{(n)}=\mathbf{A}^{(n)}(\mathbf{Z}^{(n)})^\top$ bằng cách so sánh từng phần tử. Với một bộ chỉ số $(i_1,\ldots,i_N)$, mode-$n$ matricization đặt $i_n$ làm chỉ số hàng và gom các chỉ số còn lại thành một chỉ số cột $j$. Do đó
\begin{equation}
    [\mathbf{X}_{(n)}]_{i_n,j}
    =
...
\begin{equation}
    ...
    =
    \sum_{r=1}^{R}
    A^{(1)}_{i_1,r}\cdots A^{(n)}_{i_n,r}\cdots A^{(N)}_{i_N,r}.
\end{equation}
Mặt khác, theo định nghĩa Khatri-Rao theo cột, phần tử hàng $j$, cột $r$ của $\mathbf{Z}^{(n)}$ chính là tích của các factor ở mọi mode trừ mode $n$:
\begin{equation}
    [\mathbf{Z}^{(n)}]_{j,r}
    =
    \prod_{k\neq n} A^{(k)}_{i_k,r}.
\end{equation}
Vì vậy phần tử $(i_n,j)$ của vế phải trong~\eqref{eq:cp-khatri-rao} là
\begin{align}
    [\mathbf{A}^{(n)}(\mathbf{Z}^{(n)})^\top]_{i_n,j}
    &=
    \sum_{r=1}^{R}
    A^{(n)}_{i_n,r}[\mathbf{Z}^{(n)}]_{j,r} \\
    &=
    \sum_{r=1}^{R}
    A^{(n)}_{i_n,r}\prod_{k\neq n} A^{(k)}_{i_k,r} \\
    &=
    \sum_{r=1}^{R}
    A^{(1)}_{i_1,r}\cdots A^{(N)}_{i_N,r}
    =
    [\mathbf{X}_{(n)}]_{i_n,j}.
\end{align}
Hai ma trận có cùng mọi phần tử nên bằng nhau. Thứ tự các factor trong $\mathbf{Z}^{(n)}$ chỉ quyết định quy ước đánh số cột $j$ của matricization; khi dùng cùng một quy ước cho $\mathbf{X}_{(n)}$ và $\mathbf{Z}^{(n)}$, đẳng thức luôn đúng.
\end{proof}
...
\begin{tikzpicture}[every node/.style={font=\small}]
  ...
  \node[font=\Large,gray] at (3.05,-1.0) {$\approx$};
...
\begin{enumerate}
    ...
    \begin{equation}
        \mathbf{X}_{(n)} = \mathbf{U}^{(n)} \boldsymbol{\Sigma}^{(n)} (\mathbf{V}^{(n)})^\top.
    \end{equation}
