...
\begin{tikzpicture}[scale=0.82, every node/.style={font=\small}]
  ...
  % --- unfold ---
  \draw[->, line width=1.1pt, gray!65, >=stealth] (2.75, \tfgCy-0.10) -- (4.25, \tfgCy-0.10)
    node[midway, above=3pt, font=\footnotesize, text=gray!65] {unfold};
...
\begin{tikzpicture}[scale=0.86, every node/.style={font=\small}]
  ...
  \begin{scope}[xshift=3.15cm]
    ...
    \node[text=violet!80!black, font=\footnotesize\bfseries] at (0.75, 0.25)
      {$\mathcal{X}$};
  \end{scope}
\end{tikzpicture}
\vspace{0.4em}
