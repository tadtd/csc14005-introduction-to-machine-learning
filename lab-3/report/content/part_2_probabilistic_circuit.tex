...
\end{align*}
Đây là một ví dụ điển hình cho cách biểu diễn Tucker dưới dạng circuit
...
Ba dạng truy vấn expectation thực tế thường được xét gồm
\cite{chrysos2025foundationsTutorial}:
\begin{align}
...
Query fairness so sánh hai kỳ vọng có điều kiện theo biến nhạy cảm
$A$:
\begin{equation}
...
hai factorization.  Ngược lại, khi $f$ là neural network thông thường, compatibility
không còn áp dụng; lúc đó sampling từ PC là cách xấp xỉ tự nhiên
\cite{chrysos2025foundationsTutorial}.

\begin{table}[h]
\centering
\caption{Cách nhìn tensor factorization của các query suy luận.}
...
Bốn câu hỏi nghiên cứu còn mở có thể được tóm tắt như sau
\cite{chrysos2025foundationsTutorial}:
