...
tensor factorizations (xấp xỉ tensor bằng các nhân tử bậc thấp hơn) và circuits
...
Thông điệp trung tâm của tutorial là: hai hướng này không hề độc lập với nhau.
Mỗi tensor factorization có thể được mã hóa chính xác như một circuit, và một deep
circuit tương ứng với một hierarchical tensor factorization
\cite{chrysos2025foundationsTutorial, loconte2025relationship}.  Mối liên hệ này
quan trọng theo hai chiều:
...
\begin{itemize}[leftmargin=2em]
    ...
    \item Chiều circuit $\to$ TF: các structural properties của circuit (smoothness,
    decomposability) tương ứng chính xác với điều kiện để một phân rã tensor hỗ
    trợ suy luận hiệu quả.
\end{itemize}
...
Phần này trình bày bốn mục tiêu mà tutorial xây dựng dựa trên mối liên hệ đó
\cite{chrysos2025foundationsTutorial}: (1) thống nhất các tensor factorizations
theo cách module hóa (``Lego block approach''), (2) xây các factorization mới bằng
cách ghép layers, (3) thiết kế thuật toán tractable có hệ thống dựa trên structural
...
Điểm khởi đầu của Part~II trong tutorial là quan sát sau
...
\end{equation}
Trong ngôn ngữ circuit, factor matrices cung cấp input layers (tương tự CP),
...
Tutorial định nghĩa circuit theo ba quy tắc tổ hợp, cho phép xây dựng circuit từng
...
Tutorial đặt tên ``A Lego block approach to unify tensor factorizations'': mỗi loại
factorization tương ứng với một ``block'', và ta tạo factorization mới bằng cách
...
Sau khi mã hóa tensor factorization thành circuit, câu hỏi trung tâm là: circuit đó
...
\paragraph{Scope.}
\emph{Scope} của unit $n$, ký hiệu $\mathrm{scope}(n)$, là tập biến mà hàm $c_n$
tại unit đó phụ thuộc vào.  Với unit lá, scope được xác định bởi input function.
Với sum/product unit:
\begin{equation}
...
Input functions không nhất thiết chỉ là embedding entries $f(x) = v_{x,r}$.
Loconte et al.\ chỉ ra rằng có thể thay thế chúng bằng probability mass functions
(Binomial, Categorical), continuous distributions ($\mathcal{N}(\mu,\sigma^2)$,
splines, polynomials), hoặc thậm chí các deep generative models
\cite{loconte2025relationship}. Khi input functions là hàm liên tục, tensor
tương ứng trở thành \emph{infinite-dimensional} và circuit mã hóa một PDF thay vì PMF.
...
Bài của Loconte et al.\ nhấn mạnh rằng non-negative tensor factorizations, khi
được biểu diễn như monotonic PCs, không chỉ hỗ trợ marginal inference mà còn có thể
được đọc như các mô hình sinh với biến ẩn rời rạc \cite{loconte2025relationship}.
...
\paragraph{Liên hệ thống nhất với các họ mô hình.}
Bức tranh tổng thể của tutorial là: deep circuits là một ngôn ngữ chung cho nhiều
họ mô hình \cite{chrysos2025foundationsTutorial}.  Bảng~\ref{tab:circuit-models}
tóm tắt các mối liên hệ chính.

\begin{table}[h]
\centering
\caption{Mối quan hệ giữa deep circuits, tensor factorizations và các họ mô hình
         khác \cite{chrysos2025foundationsTutorial, loconte2025relationship}.}
\label{tab:circuit-models}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Circuit / Factorization} & \textbf{Tương đương} \\
\midrule
Non-negative CP circuit (shallow)    & Mixture model bậc 1, PMF rank-$R$ \\
Non-negative Tucker circuit (shallow) & Tucker factorization không âm \\
Deep smooth+decomposable circuit      & Hierarchical Tucker, hierarchical mixtures \\
Deterministic + smooth + decomposable & Decision trees, logical formulas \& constraints \\
Circuit với CP layers + linear tree   & MPS / Tensor-Train \\
\bottomrule
\end{tabular}
\end{table}
...
với công thức chuẩn hóa dùng tích phân thay vì tổng.  Loconte et al.\ cũng chỉ ra
rằng quasi-tensors có thể được biểu diễn bằng circuits trên các biến có miền vô hạn,
và rằng việc thêm \emph{integral units} cho phép xem một lớp PC gần đây như các
factorization liên tục hoặc infinite-rank.  Đây là cơ sở đúng cho Probabilistic
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=2em]
    \item \textbf{Optimization}: lý thuyết hội tụ của shallow factorizations có thể
    mở rộng sang deep circuits không?
